...
// Validation function to check for valid numbers
...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  const _buildData = () => {
    ...
    for (let i = 0; i < arcs.length; i++) {
      ...
      const startLat = isValidNumber(arc.startLat) ? arc.startLat : 0;
      const startLng = isValidNumber(arc.startLng) ? arc.startLng : 0;
      const endLat = isValidNumber(arc.endLat) ? arc.endLat : 0;
      const endLng = isValidNumber(arc.endLng) ? arc.endLng : 0;
      const order = isValidNumber(arc.order) ? arc.order : 1;
      
      const rgb = hexToRgb(arc.color);
      
      // Use fallback color if conversion fails
      const safeRgb = rgb || { r: 203, g: 172, b: 249 }; // Purple fallback
      
      points.push({
        size: defaultProps.pointSize,
        order: order,
        color: (t: number) => `rgba(${safeRgb.r}, ${safeRgb.g}, ${safeRgb.b}, ${1 - t})`,
        lat: startLat,
        lng: startLng,
      });
    ...
    try {
      globeRef.current
        ...
        .arcStartLat((d) => {
          ...
          return isValidNumber(lat) ? lat : 0;
        })
        .arcStartLng((d) => {
          const lng = (d as { startLng: number }).startLng;
          return isValidNumber(lng) ? lng : 0;
        })
        .arcEndLat((d) => {
        ...
        .arcAltitude((e) => {
          ...
          return isValidNumber(alt) ? alt : 0.1;
        })
        .arcStroke((e) => {
        ...
        .arcDashInitialGap((e) => {
          ...
          return isValidNumber(order) ? order : 1;
        })
        .arcDashGap(15)
